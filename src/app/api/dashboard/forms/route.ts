...
export async function POST(req: Request) {
  ...
  const usage = await checkAndIncrementUsage(user.id, user.email, "filing");
  if (!usage.allowed) {
  ...
  const calcQuery = supabaseAdmin.from("tax_calculations").select("*").eq("user_id", user.id);
  ...
  await logActivity(user.id, "form_created", `Created a new ${formType} draft`);
